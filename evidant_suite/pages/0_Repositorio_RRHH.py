...
# ── Header ────────────────────────────────────────────────────────────────────
st.markdown('''<div style="display:flex;align-items:center;gap:0.8rem;margin-bottom:0.5rem;">
<div style="font-size:2rem;">D</div>
<div>
<div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:1.6rem;font-weight:800;
background:linear-gradient(135deg,#e8f0fe,#00e5ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">
Repositorio de Funcionarios</div>
<div style="font-size:0.8rem;color:#6b8caf;">RR.HH. DAP SSMC - Contratos - Trazabilidad - Validacion</div>
</div></div><hr/>''', unsafe_allow_html=True)
    ...
    if stats["total_contratos"] == 0:
        ...
        with col_a:
            ...
            if stats["por_calidad"]:
                ...
                st.bar_chart(df_cal.set_index("Calidad")["Contratos"])
        with col_b:
            ...
            if stats["por_ley"]:
                ...
                st.bar_chart(df_ley.set_index("Ley")["Contratos"])
        ...
        if todos_activos:
            ...
            with col_g:
                if not df_cc_monto.empty:
                    st.bar_chart(df_cc_monto.set_index("Centro de Costo")["Total Haberes CLP"])
            with col_t:
                st.dataframe(df_cc_monto[["Centro de Costo","Total Haberes (formato)"]],
            ...
            if "UNIDAD" in df_all.columns:
                ...
                with col_u1:
                    ...
                    st.bar_chart(df_unidad.set_index("UNIDAD")["Personas"])
                with col_u2:
                    st.caption("Total Haberes CLP por Unidad")
                    st.bar_chart(df_unidad.set_index("UNIDAD")["Total_Haberes"])

                st.dataframe(
                    df_unidad[["UNIDAD","Personas","Total Haberes (CLP)"]].rename(columns={"UNIDAD":"Unidad"}),
                    use_container_width=True, hide_index=True)
            ...
            if "PLANTA" in df_all.columns:
                ...
                with col_p1:
                    ...
                    st.bar_chart(df_planta.set_index("PLANTA")["Personas"])
                with col_p2:
                    st.caption("Total Haberes CLP por Planta")
                    st.bar_chart(df_planta.set_index("PLANTA")["Total_Haberes"])

                st.dataframe(
                    df_planta[["PLANTA","Personas","Total Haberes (CLP)"]].rename(columns={"PLANTA":"Planta"}),
                    use_container_width=True, hide_index=True)
