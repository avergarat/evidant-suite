...
<style>
  ...
  .stMultiSelect>div>div{background:var(--ev-card)!important;border-color:var(--ev-border)!important;color:var(--ev-text)!important;border-radius:8px!important;}
...
<div style="display:flex;align-items:center;gap:0.8rem;margin-bottom:0.5rem;">
  ...
  <div>
    <div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:1.6rem;font-weight:800;
                background:linear-gradient(135deg,#e8f0fe,#00e5ff);
                -webkit-background-clip:text;-webkit-text-fill-color:transparent;">
      PRIMER PASO: Consolidación Reportería Leyes SIRH
    </div>
    <div style="font-size:0.8rem;color:#6b8caf;">
      Paso 1 de 2 &nbsp;·&nbsp; Consolida Leyes 19.664 · 18.834 · 15.076 · Homologación DAP SSMC
    </div>
  </div>
</div>
<hr/>
...
if st.button("⚡  CONSOLIDAR Y HOMOLOGAR", type="primary", use_container_width=True):
    ...
    try:
        ...
        with st.spinner(f"Consolidando {len(selected_sheets)} hojas..."):
        ...
        with st.spinner("Leyendo consolidado y aplicando homologación..."):
        ...
        with st.spinner("Insertando columnas CENTRO DE COSTO y PROGRAMA..."):
        ...
        with st.spinner("Generando ID único por contrato..."):
        ...
        with st.spinner("Actualizando repositorio de funcionarios..."):
            try:
        ...
        with st.spinner("Exportando archivo con hojas de plantilla..."):
        ...
        if sin_match > 0 and sin_match_vals:
            with st.expander(
                ...
                st.dataframe(
                    pd.DataFrame({"SIRH_SIN_MATCH": sin_match_vals}),
                    use_container_width=True,
                )
                st.caption(
