...
st.set_page_config(page_title="Paso 4: Rendiciones - Evidant", page_icon="📋", layout="wide")
...
st.info("Sube **(1)** 3. REDISTRIBUCION DE GASTOS.xlsx con hoja CONSOLIDADO_REDISTRIBUIDO y **(2)** honorarios.xlsx con hoja HONORARIOS. Las reglas de homologación están embebidas.")
...
if st.button("GENERAR 4. RENDICIONES", type="primary", use_container_width=True):
    try:
        with st.status("📋 Generando Rendiciones...", expanded=True) as _rend_status:
          _pbar_r = st.progress(0, text="Iniciando...")
          with st.spinner("Paso 1/4 — Leyendo archivos..."):
            _pbar_r.progress(0.05, text="Paso 1/4 — Leyendo archivos...")
          ...
          _pbar_r.progress(0.30, text="Paso 2/4 — Transformando remuneraciones...")
          with st.spinner("Paso 2/4 — Transformando remuneraciones..."):
          ...
          _pbar_r.progress(0.55, text="Paso 3/4 — Transformando honorarios...")
          with st.spinner("Paso 3/4 — Transformando honorarios..."):
            oh = pd.DataFrame()
          ...
          _pbar_r.progress(0.80, text="Paso 4/4 — Consolidando y exportando...")
          with st.spinner("Paso 4/4 — Consolidando y exportando..."):
          ...
          _pbar_r.progress(1.0, text=f"✅ {len(df_out):,} registros generados")
          _rend_status.update(label=f"✅ Rendiciones generadas — {len(df_out):,} registros", state="complete", expanded=False)
        ...
        if n_sm_c>0 or n_sm_h>0:
            with st.expander(f"Programas sin homologacion ({n_sm_c} REMU - {n_sm_h} HON)"):
                if n_sm_c>0:
                    ...
                    st.markdown(ev_design.ev_table_html(pd.DataFrame({"PROGRAMA":vals})), unsafe_allow_html=True)
                if n_sm_h>0:
                    ...
                    st.markdown(ev_design.ev_table_html(pd.DataFrame({"NOMBRE_PROGRAMA":vals})), unsafe_allow_html=True)
        st.subheader("Vista previa -- primeras 50 filas")
        st.markdown(ev_design.ev_table_html(df_out.head(50)), unsafe_allow_html=True)
    except Exception:
