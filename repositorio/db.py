...
def _parse_haber(v) -> float:
    """Convierte TOTAL_HABER (texto con posibles puntos/comas chilenas) a float."""
    if v is None:
        return 0.0
...
def get_stats() -> dict:
    ...
    with get_conn() as conn:
        ...
        # CC con conteo + suma haberes
        cc_rows = conn.execute(
            "SELECT CENTRO_DE_COSTO, COUNT(*) as n, TOTAL_HABER FROM contratos WHERE ACTIVO=1 ORDER BY CENTRO_DE_COSTO"
        ).fetchall()
        # Agrupar en Python para sumar TOTAL_HABER (TEXT → float)
        from collections import defaultdict
        cc_agg = defaultdict(lambda: {"n": 0, "haber": 0.0})
        for r in cc_rows:
            cc_agg[r["CENTRO_DE_COSTO"]]["n"] += 1
            cc_agg[r["CENTRO_DE_COSTO"]]["haber"] += _parse_haber(r["TOTAL_HABER"])
        por_cc = sorted(
            [{"CENTRO_DE_COSTO": k, "n": v["n"], "total_haber": v["haber"]}
             for k, v in cc_agg.items() if k],
            key=lambda x: x["total_haber"], reverse=True
        )

        # UNIDAD con conteo personas únicas + suma haberes
        unidad_rows = conn.execute(
            "SELECT UNIDAD, RUT_DV, TOTAL_HABER FROM contratos WHERE ACTIVO=1"
        ).fetchall()
        unidad_agg = defaultdict(lambda: {"ruts": set(), "haber": 0.0})
        for r in unidad_rows:
            u = r["UNIDAD"] or ""
            if u:
                unidad_agg[u]["ruts"].add(r["RUT_DV"])
                unidad_agg[u]["haber"] += _parse_haber(r["TOTAL_HABER"])
        por_unidad = sorted(
            [{"UNIDAD": k, "personas": len(v["ruts"]), "total_haber": v["haber"]}
             for k, v in unidad_agg.items()],
            key=lambda x: x["total_haber"], reverse=True
        )

        # PLANTA con conteo personas únicas + suma haberes
        planta_rows = conn.execute(
            "SELECT PLANTA, RUT_DV, TOTAL_HABER FROM contratos WHERE ACTIVO=1"
        ).fetchall()
        planta_agg = defaultdict(lambda: {"ruts": set(), "haber": 0.0})
        for r in planta_rows:
            p = r["PLANTA"] or ""
            if p:
                planta_agg[p]["ruts"].add(r["RUT_DV"])
                planta_agg[p]["haber"] += _parse_haber(r["TOTAL_HABER"])
        por_planta = sorted(
            [{"PLANTA": k, "personas": len(v["ruts"]), "total_haber": v["haber"]}
             for k, v in planta_agg.items()],
            key=lambda x: x["total_haber"], reverse=True
        )
